...
function settings_form(game_manager: GameManager, player: Player) {
    ...
    form.show(player).then(r => {
        ...
        if (r.formValues) {
            ...
            game_manager.settings.grace_period_mins = Number(values[4])
            game_manager.settings.main_period_mins = Number(values[5])
            game_manager.settings.deathmatch_enabled = Boolean(values[6])
            game_manager.settings.halftime_regeneration = Boolean(values[7])
